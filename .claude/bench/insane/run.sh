...
TASKS_DIR="$REPO_ROOT/.claude/bench/insane/tasks"
CORTEX_MODEL="${CORTEX_MODEL:-deepseek-v4-pro:cloud}"
TIMEOUT_SECS="${TIMEOUT_SECS:-600}"  # 10 min per run
...
printf 'system\ttask\trun_exit\tcheck_pass\ttest_pass\tlatency_s\trust_lines\n' > "$RESULTS_TSV"
...
run_one() {
  ...
  printf '%s\t%s\t%d\t%d\t%d\t%d\t%d\n' \
    "$system" "$task" "$run_exit" "$check_pass" "$test_pass" "$latency" "$rust_lines" \
    >> "$RESULTS_TSV"

  local check_mark="FAIL"; [[ $check_pass -eq 0 ]] && check_mark="PASS"
  local test_mark="FAIL";  [[ $test_pass  -eq 0 ]] && test_mark="PASS"
  echo "  exit=$run_exit  cargo check=$check_mark  cargo test=$test_mark  latency=${latency}s  rust_lines=$rust_lines"
}
